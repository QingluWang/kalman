#ifndef KALMAN_EXAMPLES_UGV_POSITIONMEASUREMENTMODEL_HPP_
#define KALMAN_EXAMPLES_UGV_POSITIONMEASUREMENTMODEL_HPP_
...
/**
 * @brief Measurement vector measuring the UGV position
 *
 * @param T Numeric scalar type
 */
template<typename T>
class PositionMeasurement : public Kalman::Vector<T, 5>
{
public:
    KALMAN_VECTOR(PositionMeasurement, T, 5)
...
public:
    ...
    typedef  KalmanExamples::UGV::PositionMeasurement<T> M;
     ...
     * @param landmark1z The z-position of landmark 1
    ...
    PositionMeasurementModel(T landmark1x, T landmark1y,, T landmark1z, T landmark2x, T landmark2y, T landmark2z)
    {
        ...
        landmark2 << landmark2x, landmark2y, landmark2z;
    ...
    M h(const S& x) const
    {
        ...
        // UGV position as (p_x,p_y,p_z)-vector
        // This uses the Eigen template method to get the first 3 elements of the vector
        Kalman::Vector<T, 3> position = x.template head<3>();
        
        measurement.p_x() = position(0);
        measurement.p_y() = position(1);
        measurement.p_z() = position(2);
        
        // Distance of robot to landmark 1
        Kalman::Vector<T, 3> delta1 = position - landmark1;
        measurement.d1() = std::sqrt( delta1.dot(delta1) );
        
        // Distance of robot to landmark 2
        Kalman::Vector<T, 3> delta2 = position - landmark2;
        measurement.d2() = std::sqrt( delta2.dot(delta2) );
        
        return measurement;
    }
protected:
    //! Position of landmark 1 given as (x,y,z)-measurement
    Kalman::Vector<T, 3> landmark1;
    
    //! Position of landmark 2 given as (x,y,z)-measurement
    Kalman::Vector<T, 3> landmark2;
